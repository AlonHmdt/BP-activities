...
export default {
  ...
  computed: {
   itemsList() {
      ...
      const itemsAfetFilter = this.items.filter(item => this.selectedCategories.indexOf(item.resource_type) > -1 &&  (item.topic_data.name.toLowerCase() + ' ' + item.resource_type.toLowerCase()).includes(this.keyword.toLowerCase()));
