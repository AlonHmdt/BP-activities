...
export default {
  ...
  computed: {
   itemsList() {
      if (this.selectedCategories.indexOf('all') > -1 && this.keyword === '') {
          return this.items;
        }
      if (this.selectedCategories.indexOf('all') > -1 && this.keyword !== '') {
        return this.items.filter(item => (item.topic_data.name.toLowerCase() + ' ' + item.topic_data.name.toLowerCase()).includes( this.keyword.toLowerCase()));
      }
        const itemsAfetFilter = this.items.filter(item => this.selectedCategories.indexOf(item.resource_type) > -1);
        return itemsAfetFilter;
      }
   }
}
</script>
