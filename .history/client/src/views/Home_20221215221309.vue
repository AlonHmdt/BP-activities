...
export default {
  ...
  methods: {
    ...
    setIconPath() {
      ...
      const imagesContext = require.context("../assets/topics", false);
      const images = imagesContext.keys().map(imagesContext);
      // return images.find((img) =>
      //   img.includes(this.item.topic_data.name.toLowerCase())
      // );
      console.log(images);
      // return imagesContext(".." + this.item.topic_data.icon_path);
      return "/img/dogs.a51dc96e.png";
    },
    generateCategories() {
      const categories = {};
      ...
      vm.items = response.data.map((item) => {
        return {
          ...
          topic_data: {
            ...
            icon_path: vm.setIconPath(item.topic_data.name),
          },
        };
      });
