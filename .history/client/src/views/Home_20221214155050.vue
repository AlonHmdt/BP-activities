  ...
  methods: {
   normalizeDate: (item) => {
      ...
      generateCategories() {
         ...
         const categoryLabel = (categoryLabelArray.indexOf('-') > -1 || categoryLabelArray.indexOf('Make') > -1) ? categoryLabelArray.join('-') :  categoryLabelArray.join(' ');
     ...
     const response = await axios.get('http://localhost:3000/activities/v1');
      next(vm => {
         ...
         vm.items = response.data.map(item => {
           ...
           topic_data: {...item.topic_data, name:vm.firstLetterToUpperCase(item.topic_data.name), icon_path: require(`../../../assets/topics/adalovelace.png`)}            
         } 
         });
