   ...
   },
   {
    ...
    this.items.map(item => {
      ...
      for (const word of strArray) {
         categoryLabelArray.push(word.charAt(0).toUpperCase() + word.slice(1));
      }
      const categoryLabel = (categoryLabelArray.indexOf('Word') || categoryLabelArray.indexOf('Make')) > 0 ? categoryLabel.join('-') :   categoryLabel.join('-')
      this.categoriesMap[item.resource_type] = categoryLabel.join(' ');; 
    });
