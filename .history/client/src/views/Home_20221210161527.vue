...
export default {
  ...
  created() {
   ...
   },
   {
    ...
    this.itmes.map(item => {
      let categoryLabelSplitted = item.resource_type.split('_');
      categoryLabelSplitted = categoryLabelSplitted.map(item => item.charAt(0).toUpperCase())
      this.categoriesMap[item.resource_type] = item.resource_type; 
    });
  }
}
</script>
