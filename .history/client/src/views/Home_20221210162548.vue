   ...
   },
   {
    ...
    this.items.map(item => {
      ...
      for (const word of strArray) {
         categoryLabel.push(word.charAt(0).toUpperCase() + word.slice(1));
      }
      let str = item.resource_type;
      let categoryLabel = str.replace(/(?:_| |\b)(\w)/g, function(str, p1) { return p1.toUpperCase()});
