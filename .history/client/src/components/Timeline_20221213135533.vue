<template>
    <div class="timeline">
        <div v-for="(month,index) in getItems" :key=monthIndex>
          <span class="month-label">
                {{monthsLabel[month.month_id]}}
              </span>
              <div class="divider"></div>
            <div v-for="(item,index) in month.items" :key=itemIndex>
              <TimelineItem :item="item" :categoriesMap="categoriesMap"/>
              <div class="divider" v-if="getItems.length === index">{{index}}</div>
            </div>
  	      </div>
    </div>
  </template>
