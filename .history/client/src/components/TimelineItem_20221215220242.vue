...
export default {
  ...
  computed: {
    getImg() {
      ...
      const imagesContext = require.context("../assets/topics", false, /.png$/);
      const images = imagesContext.keys().map(imagesContext);
      console.log(images.find((img) => img.includes(this.item.name)));
      return images.find((img) => img.includes(this.item.name));
      // return require(".." + this.item.topic_data.icon_path);
    },
  },
};
</script>
